...
const rows = [
  ...
  ["提供時期", "決済完了後、対象Watchを有料機能へ切り替えます。"],
  ["解約", "次回更新日前までStripe Customer Portalから解約できます。解約後も当該請求期間末まで利用できます。"],
  ["返金", "デジタルサービスの性質上、提供開始後の利用済み期間は原則返金しません。法令上必要な場合を除きます。"],
  ["動作環境", "最新の主要デスクトップ・モバイルブラウザ。AI Providerや対象サイトの状態により一部観測が失敗する場合があります。"],
];

export default function CommercePage() {
  return <MarketingShell eyebrow="COMMERCIAL DISCLOSURE" title="特定商取引法に基づく表記。" lead="有料Watchの販売者、価格、更新、解約、返金条件を表示します。">
    ...
    <h2>サービスの性質</h2><p>AIX Nextは外部AIの回答を観測・分析するサービスです。特定の順位、推薦、Citation、問い合わせ、契約、売上を保証しません。測定条件と制約は測定方法ページに表示します。</p>
  </MarketingShell>;
}
