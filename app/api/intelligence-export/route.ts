...
export async function GET(request: Request) {
  const url = new URL(request.url); const token = url.searchParams.get("token") || ""; const format = url.searchParams.get("format") || "json";
  ...
  if (format === "csv") {
    const rawScope = url.searchParams.get("scope") || "prompts"; const scope = ["prompts", "citations", "competitors", "history"].includes(rawScope) ? rawScope as "prompts" | "citations" | "competitors" | "history" : "prompts";
