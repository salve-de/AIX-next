...
export async function POST(request: Request) {
  ...
  try {
    ...
    token = body.token || ""; packId = body.packId || "unknown";
    if (!token || !body.packId || !Number.isInteger(body.pageId) || Number(body.pageId) <= 0) return Response.json({ error: "token、packId、pageIdが必要です。" }, { status: 400 });
