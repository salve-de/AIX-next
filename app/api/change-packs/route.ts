...
export async function POST(request: Request) {
  try {
    ...
    if (!body.actionId) return Response.json({ error: "actionIdが必要です。" }, { status: 400 });
    ...
    const updated = await saveChangePack(token, pack);
...
export async function PATCH(request: Request) {
  try {
    ...
    const token = resolveWatchToken(request, body.token);
    if (!token) return Response.json({ error: "Watch sessionが必要です。" }, { status: 401 });
    if (!body.packId || !body.status) return Response.json({ error: "packIdとstatusが必要です。" }, { status: 400 });
    ...
    const updated = await saveChangePack(token, next);
