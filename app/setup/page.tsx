...
export default function SetupPage() {
  ...
  const rows = [
    ...
    ["Supabase / Scan・Watch・Change Pack永続化", Boolean(env.supabaseUrl && env.supabaseServiceKey)],
  ...
  return <MarketingShell eyebrow="PRODUCTION READINESS" title="AIX Nextの本番設定。" lead="秘密値そのものは表示せず、診断 → Watch → Change Pack → 再測定まで商品として稼働できる設定が揃っているかだけを確認します。">
    ...
    <p>必要migrationは<code>001_core.sql</code>〜<code>009_watch_change_pack.sql</code>です。004以降でWatch重複防止、Stripe ID保存、ジョブlease、分割測定、最終反映の原子化、Change Pack永続化を追加しています。</p>
