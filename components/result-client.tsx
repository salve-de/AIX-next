...
export function ResultClient() {
  ...
  async function startWatch(event: FormEvent) {
    ...
    } catch (caught) { setError(caught instanceof Error ? caught.message : "継続モニタリングを開始できませんでした。"); }
  ...
  if (!hasMeasurement) return <main className="ux2-report-page">
    <SiteHeader compact />
  ...
  function QuestionRow({ loss }: { loss: LostPrompt }) {
  ...
  return <main className="ux2-report-page">
    ...
    <section className="ux2-report-top">
      <div className="shell">
        <div className="ux2-report-breadcrumb"><span>{sample ? "架空サンプル" : "診断結果"} · {result.discovery.brandName}</span><Link href="/#scan">別の会社を診断</Link></div>
    ...
    <section className="ux2-report-section white" id="questions"><div className="shell">
      ...
      <div className="ux2-question-table">
        <div className="ux2-question-row header"><div>比較質問</div><div>自社</div><div>最も選ばれた競合</div><div>主な情報差</div></div>
        {visibleLosses.map((loss) => <QuestionRow key={loss.promptId} loss={loss} />)}
      </div>
      {remainingLosses.length ? <details className="ux2-spec-details"><summary>残り{remainingLosses.length}件を見る</summary><div className="ux2-question-table">{remainingLosses.map((loss) => <QuestionRow key={loss.promptId} loss={loss} />)}</div></details> : null}
    </div></section>
    ...
    <section className="ux2-report-section" id="competitors"><div className="shell">
      ...
      <div className="ux2-competitor-list">
        <div className="ux2-competitor-table">
          {result.competitors.slice(0, 6).map((competitor, index) => <div key={competitor.name}><span>{index + 1}</span><strong>{competitor.name}</strong><div className="ux2-bar"><i style={{ width: `${competitor.coverage}%` }} /></div><span>{competitor.recommendedCount}/{result.successfulObservations}</span></div>)}
          <div className="own"><span>{result.marketPosition}</span><strong>{result.discovery.brandName}</strong><div className="ux2-bar"><i style={{ width: `${result.recommendationCoverage}%` }} /></div><span>{result.ownRecommendationCount}/{result.successfulObservations}</span></div>
        </div>
        <aside className="ux2-competitor-focus">
          <small>最も重要な候補外質問</small>
          <h3>「{primaryLoss?.prompt || "候補外質問なし"}」</h3>
          <p>{primaryLoss?.summary || "今回の主要質問では候補に入っています。"}</p>
          {primaryLoss?.citations.length ? <ul>{primaryLoss.citations.slice(0, 4).map((citation) => <li key={citation.url}><a href={citation.url} target="_blank" rel="noreferrer">{citation.title || citation.domain}</a></li>)}</ul> : null}
        </aside>
      </div>
    </div></section>
    ...
    {firstAction ? <section className="ux2-report-section"><div className="shell">
      <div className="ux2-report-heading"><div><p className="ux2-label">最優先の改善</p><h2>全部直さず、まずこの1件。</h2><p>順位上昇を保証するものではなく、今回観測した比較材料の差から最初に検証する変更です。</p></div></div>
      <div className="ux2-next-action"><small>最初に検証する変更</small><h2>{firstAction.title}</h2><p>{firstAction.rationale}</p><div className="ux2-action-meta"><span>{firstAction.relatedPromptCount}質問に関連</span><span>対象: {firstAction.target}</span><span>確度: {confidenceLabel(firstAction.confidence)}</span></div></div>
    </div></section> : null}

    <section className="ux2-watch-cta" id="watch"><div className="shell ux2-watch-cta-grid">
      <div><p className="ux2-label">14日間無料</p><h2>直したあと、同じ質問で変化を確認する。</h2><p>候補入りが増えた質問、まだ候補外の質問、競合・引用元の変化を再測定します。無料期間から自動課金はありません。</p></div>
      <form className="ux2-watch-form" onSubmit={startWatch}><label htmlFor="watch-email">会社メール</label><input id="watch-email" type="email" required value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@company.jp" /><button className="button button-accent" disabled={watchBusy}>{watchBusy ? "開始しています…" : "14日間無料で追跡"}<ArrowIcon /></button><small>カード登録不要。会社メールで結果を保存します。</small></form>
    </div></section>
