    ...
    <section className="result-hero">
      <div className="shell result-hero-grid">
        <div className="result-verdict">
          ...
          {hasMeasurement ? <><h1>{result.discovery.brandName}は、<br /><span>AI比較で{result.marketSize}社中{result.marketPosition}位。</span></h1><p>{result.panel.promptCount}の購買質問のうち、候補に入ったのは<strong>{shortlistedPromptCount}問</strong>、候補外は<strong>{result.lostPrompts.length}問</strong>でした。</p></> : <><h1>市場は特定しました。<br /><span>AI観測は未完了です。</span></h1><p>Provider設定後に、同じBuyer PromptをOpenAI・Gemini・Perplexityで測定します。</p></>}
        ...
        <div className="decision-map">
          ...
          <div className="decision-diagnosis"><EvidenceIcon /><p><small>確認できないEvidence</small><strong>{result.evidenceGaps[0]?.label || "Evidence差を解析中"}</strong></p><span>{result.evidenceGaps[0]?.relatedPromptCount || 0} prompts</span></div>
        </div>
      </div>
    </section>

    <section className="result-section result-lost-section shell">
      <div className="section-heading"><p className="eyebrow">EXCLUDED BUYER PROMPTS</p><h2>どの購買質問で、候補から外れたか。</h2><p>{result.lostPrompts.length} / {result.panel.promptCount}問で、過半数のAI回答が自社を購入候補に入れませんでした。</p></div>
