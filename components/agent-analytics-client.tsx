  ...
  return <main className="agent-page">
    ...
    {summary ? <>
      <section className="agent-kpis">
        <article><BotIcon /><small>AI crawler visits</small><strong>{summary.crawlerVisits}</strong><span>実crawler event</span></article>
        <article><ArrowIcon /><small>AI referral visits</small><strong>{summary.referralVisits}</strong><span>AI referrer event</span></article>
        <article><TrendIcon /><small>Observed conversions</small><strong>{summary.conversions}</strong><span>送信側で記録された成果</span></article>
        <article><TrendIcon /><small>Observed value</small><strong>{money(summary.observedValue, summary.currency)}</strong><span>{summary.currency ? "同一通貨だけを合算" : "複数通貨は合算しない"}</span></article>
      </section>
      <section className="agent-grid">
        <article><header><strong>Agents / Referrers</strong><span>event / conversion / value</span></header>{summary.agents.map((item) => <div className="agent-row" key={`${item.kind}-${item.agent}`}><span className={item.kind}>{item.kind}</span><strong>{item.agent}</strong><i><b style={{ width: `${Math.min(100, item.events / Math.max(1, summary.crawlerVisits + summary.referralVisits) * 180)}%` }} /></i><em>{item.events}</em><small>{item.conversions ? `${item.conversions} conv. · ${money(item.observedValue, summary.currency)}` : ""}</small></div>)}</article>
        <article><header><strong>Top landing pages</strong><span>crawler / referral / conversion</span></header>{summary.pages.map((page) => <div className="agent-page-row" key={page.path}><strong>{page.path}</strong><span>{page.crawlerVisits}</span><span>{page.referralVisits}</span><span>{page.conversions}</span><small>{page.observedValue ? money(page.observedValue, summary.currency) : ""}</small></div>)}</article>
      </section>
