...
export function ScanForm({ compact = false }: { compact?: boolean }) {
  ...
  return <form className={`scan-form ${compact ? "scan-form-compact" : ""}`} id={compact ? undefined : "scan"} onSubmit={submit} noValidate>
    <div className="scan-field"><SearchIcon /><input aria-label="会社サイトURL" autoCapitalize="none" autoCorrect="off" inputMode="url" placeholder="example.com" value={url} onChange={(event) => setUrl(event.target.value)} /><button type="submit"><span>無料で診断</span><ArrowIcon /></button></div>
