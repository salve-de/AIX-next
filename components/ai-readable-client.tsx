...
function demoDraft(domain: string): AiReadableDraft {
  return {
    ...
    llmsTxt: `# ${domain}\n\n> 会社が確認した公開情報を、AIが読み取りやすい順番に整理した下書きです。\n\n## 参照元\n\n- [公開サイト](https://${domain}) — 実際に確認したページの内容だけを掲載する\n\n## 公開前に確認すること\n\n- 会社名・サービス名・説明が参照元ページと一致しているか確認する。\n- 料金・実績・導入期間などの数値を原典と照合する。\n- 掲載するページ、権限、更新担当者を確認する。\n`,
    ...
    sourcePages: [
      { url: `https://${domain}`, title: "公開サイト", description: "実際に確認したページの内容を掲載します" },
    ],
    publishChecks: [
...
export function AiReadableClient() {
  ...
  return <main className="document-page ai-info-page">
    ...
    <section className="document-body shell ai-info-body">
      ...
      {!draft ? <section className="ai-info-empty"><p className="overline">下書きの作成</p><h2>まずWatchで、測定結果と参照元を確認します。</h2><p>有料Watchでは、実際に取得できた公開ページだけを使って、公開情報の下書きを作成できます。作成後はこの専用画面で内容を確認し、ファイルとして取得できます。</p><Link className="button button-primary" href={token ? `/watch?token=${encodeURIComponent(token)}` : "/result?sample=1"}>Watchへ戻る <ArrowIcon /></Link></section> : <>
