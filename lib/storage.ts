...
const globalMemory = globalThis as unknown as {
  ...
  aixNextStripeEvents?: Map<string, "processing" | "processed">;
};
...
const stripeEvents = globalMemory.aixNextStripeEvents ?? new Map<string, "processing" | "processed">();
globalMemory.aixNextScans = scans;
globalMemory.aixNextWatches = watches;
globalMemory.aixNextStripeEvents = stripeEvents;
...
function watchFromRow(row: any): WatchRecord {
  return {
    ...
    customLatest: row.custom_latest || null,
    customHistory: row.custom_history || [],
    evidence: row.evidence || [],
    changePacks: row.change_packs || [],
    domainClaim: row.domain_claim || null,
...
export async function getWatchById(watchId: string) {
  if (durable()) {
    const rows = await supabase<any[]>(`aix_next_watches?id=eq.${encodeURIComponent(watchId)}&limit=1`);
    return rows[0] ? watchFromRow(rows[0]) : null;
  }
  return [...watches.values()].find((watch) => watch.id === watchId) || null;
}

type WatchPatch = Partial<Pick<WatchRecord,
  "status" | "paid" | "baseline" | "latest" | "history" | "discoveryLatest" | "discoveryHistory" | "customPrompts" | "customLatest" | "customHistory" | "evidence" | "changePacks" | "domainClaim" | "executions" | "stripeCustomerId" | "stripeSubscriptionId" | "stripePriceId" | "trialEndsAt" | "nextRunAt"
>>;
export async function updateWatch(token: string, patch: WatchPatch) {
  ...
  if (durable()) {
    ...
    if (patch.customPrompts !== undefined) body.custom_prompts = patch.customPrompts;
    if (patch.customLatest !== undefined) body.custom_latest = patch.customLatest;
    if (patch.customHistory !== undefined) body.custom_history = patch.customHistory;
    ...
    if (patch.domainClaim !== undefined) body.domain_claim = patch.domainClaim;
    if (patch.executions !== undefined) body.executions = patch.executions;
...
export async function addExecution(token: string, execution: ExecutionRecord) {
  const watch = await getWatch(token);
  if (!watch) return null;
  return updateWatch(token, { executions: [...(watch.executions || []), execution].slice(-100) });
}

export async function beginStripeEvent(eventId: string, eventType: string) {
  if (!eventId) return false;
  if (durable()) {
    const rows = await supabase<any[]>("aix_next_stripe_events?on_conflict=event_id", {
      method: "POST",
  ...
  stripeEvents.set(eventId, "processing");
  return true;
}
export async function completeStripeEvent(eventId: string) {
  if (durable()) {
    await supabase(`aix_next_stripe_events?event_id=eq.${encodeURIComponent(eventId)}`, { method: "PATCH", body: JSON.stringify({ status: "processed", processed_at: new Date().toISOString() }) });
    return;
  }
  stripeEvents.set(eventId, "processed");
}
export async function releaseStripeEvent(eventId: string) {
  if (durable()) {
    await supabase(`aix_next_stripe_events?event_id=eq.${encodeURIComponent(eventId)}&status=eq.processing`, { method: "DELETE" });
    return;
  }
  if (stripeEvents.get(eventId) === "processing") stripeEvents.delete(eventId);
}
