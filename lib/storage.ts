...
import type { BuyerPrompt, ChangePack, EvidenceAnswer, ScanRecord, ScanResult, WatchRecord } from "@/lib/types";
...
type WatchPatch = Partial<Pick<WatchRecord, "status" | "paid" | "baseline" | "latest" | "history" | "discoveryLatest" | "discoveryHistory" | "customPrompts" | "customLatest" | "customHistory" | "evidence" | "changePacks" | "trialEndsAt" | "nextRunAt">>;
export async function updateWatch(token: string, patch: WatchPatch) {
  const updatedAt = new Date().toISOString();
  if (durable()) {
    const body: Record<string, unknown> = { updated_at: updatedAt };
    if (patch.status !== undefined) body.status = patch.status; if (patch.paid !== undefined) body.paid = patch.paid; if (patch.baseline !== undefined) body.baseline = patch.baseline; if (patch.latest !== undefined) body.latest = patch.latest; if (patch.history !== undefined) body.history = patch.history; if (patch.discoveryLatest !== undefined) body.discovery_latest = patch.discoveryLatest; if (patch.discoveryHistory !== undefined) body.discovery_history = patch.discoveryHistory; if (patch.customPrompts !== undefined) body.custom_prompts = patch.customPrompts; if (patch.customLatest !== undefined) body.custom_latest = patch.customLatest; if (patch.customHistory !== undefined) body.custom_history = patch.customHistory; if (patch.evidence !== undefined) body.evidence = patch.evidence; if (patch.changePacks !== undefined) body.change_packs = patch.changePacks; if (patch.trialEndsAt !== undefined) body.trial_ends_at = patch.trialEndsAt; if (patch.nextRunAt !== undefined) body.next_run_at = patch.nextRunAt;
