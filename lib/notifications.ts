...
function sessionLink(watch: WatchRecord, next: string) {
  const base = env.siteUrl.replace(/\/$/, "");
  return `${base}/api/session/exchange?token=${encodeURIComponent(watch.token)}&next=${encodeURIComponent(next)}`;
}
...
export async function sendWeeklyBrief(watch: WatchRecord, previous: ScanResult, latest: ScanResult) {
  ...
  const workspaceUrl = sessionLink(watch, "/workspace");
  const watchUrl = sessionLink(watch, "/watch");
