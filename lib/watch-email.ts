...
export async function sendWatchStarted(watch: WatchRecord) {
  ...
  const subject = `[Rovan] ${brand}のAI推薦を再確認します`;
...
export async function sendWatchUpdate(watch: WatchRecord, previous: ScanResult, options: { trialEnded?: boolean } = {}) {
  ...
  const subject = comparable ? `[Rovan] ${brand}: AIの推薦 ${previous.marketPosition}位 → ${latest.marketPosition}位` : `[Rovan] ${brand}: 今回の結果を新しい基準にしました`;
