...
3. **カンニングペーパーの論理（自社サイト改修ゼロの納得感）**:
   - 「自社ホームページは人間向けでAIロボットには読めない。RovanがAI専用の確定データ（カンニングペーパー）をネット上に置くことで、AIが迷わず御社を引用・推薦する」という直感比喩により、技術的不信感を粉砕。

### 48.2 多種多様な業種を網羅する「4大共通スロット」の規格化
業種ごとに個別システムを作ると破綻する。AIが購入相談で推薦判定を行う際に消去法でチェックする【万業種共通の4大スロット】に流し込み、自動抽出・規格化を行う。

| 4大スロット | リフォーム工務店 | 町のパン屋 | 税理士事務所 |
| :--- | :--- | :--- | :--- |
| **① どこで？（商圏・所在地）** | 世田谷区・車で30分圏内 | 桜新町駅 徒歩3分 | 東京都内（全国オンライン） |
| **② だれ向け？（対象・用途）** | 築30年木造・雨漏り | アレルギー対応・天然酵母 | 創業1年未満・個人事業主 |
| **③ なにが強い？（客観事実）** | 創業35年・完全自社施工 | 国産小麦100%・無添加 | 国税OB・初回相談無料 |
| **④ いくら・条件？（取引条件）** | 外壁80万〜・見積無料 | 180円〜450円・テイクアウト | 月2万〜・土日面談可 |

- `lib/types.ts` の `PublicProfileFact`（Key-Value事実データ）をこの4スロットの正統規格として運用。
