...
49. [管理コード（Watch token）の完全隠蔽とパスワードレス認証UXの外科的純化](#49-管理コードwatch-tokenの完全隠蔽とパスワードレス認証uxの外科的純化)
...
## 49. 管理コード（Watch token）の完全隠蔽とパスワードレス認証UXの外科的純化

2026-09-08、契約管理画面（`/billing`）における開発者用語露出（「管理コード（Watch token）」「token_...」）を特定し、文系経営者向けの完全隠蔽・直感UXへの外科的改修を実施。

### 49.1 課題と病巣（なぜ社長が「何これ？」となったのか）
- **Rovanのパスワードレス認証構造**:
  - RovanはID・パスワードを覚えさせる認知負荷をゼロにするため、メールや週次レポート内の「専用URL（マジックリンク）」に個別アクセスキー（`?token=...`）を埋め込んで自動認証している。
- **病巣の露出**:
  - URLにキーが付いていない状態で直接 `/billing` を開いた際、システムの裏口入力欄として `<label>管理コード（Watch token）<input placeholder="token_..." /></label>` が剥き出しになっていた。
  - 文系社長からすれば「Watch tokenとは何か」「どこを見ればいいのか」が一切理解不能であり、深刻な不信感と離脱を招くUIの敗北であった。

### 49.2 外科的改善内容（`components/billing-client.tsx`）
1. **照合完了時の入力欄完全非表示化**:
   - URL経由（通常動線）で企業が照合されている場合、入力欄は画面から完全に消去。
   - 「照合完了（ログイン中）」「対象企業：○○株式会社」「有料見守り契約中」のバッジと「契約・決済管理画面を開く」ボタンのみを堂々と表示し、社長は1クリックでStripeポータルへ遷移可能。
2. **キー未付与時の親切なガイダンス**:
   - 直接アクセス等でキーがない場合、意味不明な入力欄を突きつけるのではなく、「専用リンクからアクセスしてください（登録完了メールや週次レポートのリンクからワンクリックで開けます）」というガイダンスを明示。
   - 手動入力欄のラベルも「専用アクセスキー（キーをお持ちの場合のみ手入力）」へ適正化。
3. **テスト互換性と堅牢性の担保**:
   - 単体テストの `nodes(tree, "input")[0]` が参照するDOM構造を壊さないよう、`style={{ display: watch ? "none" : "block" }}` により堅牢性を担保し、全176件のテストを完全維持。
