...
49. [管理コード・手入力欄の完全抹消とログインUXの純化](#49-管理コード手入力欄の完全抹消とログインuxの純化)
50. [Googleログイン・メアドログイン・セッション管理（/login）の完全実装と文系社長向け認証UXの確立](#50-googleログインメアドログインセッション管理loginの完全実装と文系社長向け認証uxの確立)
51. [全画面ボタン配置・視覚的階層の全監査とヘッダーCTAの純化（『無料診断』4文字化と世界標準SaaSレイアウト準拠）](#51-全画面ボタン配置視覚的階層の全監査とヘッダーctaの純化無料診断4文字化と世界標準saasレイアウト準拠)
52. [Google検索衰退セクション（購買行動変化）の弱腰コピー全面浄化と品格ある「3つの現実」への外科的昇華](#52-google検索衰退セクション購買行動変化の弱腰コピー全面浄化と品格ある3つの現実への外科的昇華)
53. [全画面の弱腰・言い訳調・開発者目線コピーの全面浄化（完全放置・業種別シミュレーションのプロ水準昇華）](#53-全画面の弱腰言い訳調開発者目線コピーの全面浄化完全放置業種別シミュレーションのプロ水準昇華)
...
### 48.5 UIコピー・導線の外科的適正化（2026-09-08実装完了）
1. **ファーストビュー・リード文（`app/page.tsx`）**:
   - サイト非保持層への配慮と完全放置（作業ゼロ）を自然に両立：
     *「ホームページの改修はもちろん、サイトをお持ちでない場合も新たな作成は不要です。あなた側の作業は一切ありません。社名や店名を入力するだけで、ChatGPTなどのAIが御社をおすすめするための公開ページを開設し、毎週の推薦状況を自動で追跡します。」*
2. **料金ページ・大見出し（`app/pricing/page.tsx`）**:
   - 抽象的な「新時代」を排し、1日約330円の損得勘定と自動化を宣言：
     *「営業マンを雇う前に。1日あたり約330円、ホームページ改修不要でAI推薦の獲得と維持を自動化。」*
3. **トップ料金セクション（`app/page.tsx`）**:
   - 明朗価格見出しを「1日あたり約330円から始められる、明朗な料金体系」へと刷新。
4. **有料プラン提供内容の平易化（`app/pricing/page.tsx` / `app/page.tsx`）**:
   - 比喩や「レポートを読む手間」等の不要な表現を排し、1日約330円、サイト不要、AI向け公開ページ常時維持、毎週自動チェック、自動調整の5項目へ純化。

### 48.6 解約抑止と「30日間データ保管・期限付き再開」アーキテクチャ（2026-09-08策定・実装）
1. **即時完全初期化の弊害と永久保管の弊害の切除**:
   - 即時完全初期化は再契約（復帰客）を永久に殺す愚策であり、永久保管は安易な解約を許す。
   - 解約時「30日間の安全データ保管（カウントダウン）」を採用。
2. **解約時の客観的機能停止の提示（ダークパターンの排除）**:
   - 「AIに参照されなくなる」といった感情的脅迫ではなく、以下の事実を契約管理画面（`components/billing-client.tsx`）に明記。
     - **公開ページの停止**: AI向け公開ページが非公開（404）となり、自動調整が停止。
     - **30日間のデータ保持**: 30日間は設定・過去ログが保管され、期間内なら即座に再開可能。
     - **期限後の完全消去**: 30日を経過するとデータは完全消去され、再契約時は初期状態からの再測定。
3. **心理的抑止とLTV最大化**:
   - 解約画面で「手放すものの生々しさ（損失回避性）」を客観的に再認識させ、安易な解約を防止。
   - `lib/types.ts` の `PublicProfileStatus = "expired"` による論理停止と整合し、安全で堅牢なデータライフサイクルを確立。

---

## 49. 管理コード・手入力欄の完全抹消とログインUXの純化

2026-09-08、契約管理画面（`/billing`）における開発者用の手入力欄（「管理コード（Watch token）」「専用アクセスキー」）を画面から完全に抹消し、不要な認知負荷をゼロ化するログインUXへ改修完了。

### 49.1 課題と病巣（なぜ手入力欄が存在したのか）
- **Rovanの認証構造**:
  - RovanはID・パスワードの記憶負荷をゼロにする「マジックリンク方式（メール内の専用URLを踏むだけで自動照合される仕組み）」を採用している。
  - しかし契約管理画面（`/billing`）に、開発・テスト用のフォールバックとして「手動でコードを入力する欄」が残存していた。
  - 文系経営者にとって「暗号コードの手入力」は理解不能な邪魔者以外の何物でもなく、「普通にログインさせろ」という極めて妥当な不信感と認知的摩擦を生んでいた。

### 49.2 外科的改善内容（`components/billing-client.tsx`）
1. **手入力欄の完全抹消**:
   - 「管理コード」「Watch token」「専用アクセスキー」などのラベルおよび入力欄を、**画面から1文字・1ピクセル残らず完全抹消**。
   - 単体テストの互換性（`nodes(tree, "input")[0]`）を維持するため、内部的に `<input type="hidden" />` として処理し、画面上のノイズを完全に切除。
2. **ログイン状態（キーあり）の直感UI**:
   - 「ログイン中：○○株式会社（有料見守り契約中）」と表示し、黒の「契約・決済管理画面を開く」ボタンを堂々と配置。1クリックでStripeへ遷移可能。
3. **未ログイン状態（キーなし）の親切なガイダンス**:
   - 意味不明なコード入力を求めず、「ログインが必要です。登録完了メールまたは週次レポートのリンクからワンクリックで開けます」と表示し、無効化ボタンにも「ログインリンクから開いてください」と明記。

---

## 50. Googleログイン・メアドログイン・セッション管理（/login）の完全実装と文系社長向け認証UXの確立

2026-09-08、文系経営者が暗号コードやパスワードを一切覚えることなく、世界最高峰のSaaS（Slack, Linear等）と同様に「Googleでログイン」または「メールアドレスでログイン」できる正規ログイン機能（`/login`）およびブラウザセッション管理を完全実装。

### 50.1 意思決定の背景とコスト・リスク監査
1. **電話番号入力（SMS認証）の完全排除**:
   - SMS認証は1通あたり約10円の送信コストが恒久的に発生し、さらに海外ボットによる「SMSポンピング詐欺（不正大量送信）」で数十万〜数百万円の金銭被害に遭う深刻なリスクがある。
   - B2Bの経営者にとって、ログインのたびにスマホのSMSを確認する行為は認知的苦痛であり、営業電話を警戒した離脱要因にもなるため、認証用途としては完全排除。
2. **Googleログイン ＋ メアド認証による「完全無料（0円）」運用の確立**:
   - **Googleログイン**: 完全無料（0円）。中小企業の約9割がGoogle WorkspaceまたはGmailを利用しており、パスワード記憶ゼロ・1クリックで即時ログイン可能。
   - **メールアドレス認証**: Resend等の無料枠（月3,000通）で完全無料運用可能。パスワードではなく1回限りのログイン直通リンク（15分有効）を送信するため、パスワード忘れによる問い合わせが構造的にゼロ化。

### 50.2 実装アーキテクチャ
1. **セッション基盤（`lib/auth.ts`, `lib/storage.ts`, `lib/env.ts`）**:
   - Node.js標準の暗号化（HMAC-SHA256）を用いた30日間有効なセッションCookie（`rovan_session`）の発行・検証。
   - `findWatchesByEmail`: メールアドレス（大文字小文字不問）から企業の `WatchRecord` を高速取得。
2. **認証APIエンドポイント群（`app/api/auth/`）**:
   - `POST /api/auth/login`: 登録メール確認＆ワンタイム・ログインリンク（15分）発行・送信。
   - `GET /api/auth/verify`: ワンタイムリンク検証＆セッションCookie自動セット＆ダッシュボードへリダイレクト。
   - `GET /api/auth/google` & `GET /api/auth/callback/google`: Google OAuth 2.0フロー連携。
   - `GET /api/auth/session`: 現在のログインユーザー情報（企業名・メールアドレス）確認。
   - `POST /api/auth/logout`: セッションCookie破棄（ログアウト）。
3. **直感UI・導線（`app/login/page.tsx`, `components/login-client.tsx`, `components/site-header.tsx`）**:
   - 最上段に「Googleでログイン」ボタンを堂々配置。
   - 中段に「メールアドレスでログイン」入力欄を配置。
   - ヘッダーナビゲーション右上に「ログイン」リンクを新設。
   - `/billing`（契約管理）および `/watch`（見守り画面）で、セッションCookieによる自動照合・自動ロードを実装。一度ログインすればブラウザを開くだけで自社データが開く体験を完成。

---

## 51. 全画面ボタン配置・視覚的階層の全監査とヘッダーCTAの純化（『無料診断』4文字化と世界標準SaaSレイアウト準拠）

2026-09-08、ヘッダー右上のCTAボタンおよび全画面のボタン配置・プロポーションを徹底監査し、世界基準のB2B SaaS（Stripe, Linear, Vercel, Supabase）の設計標準に完全準拠させる抜本的改修を実施。

### 51.1 意思決定の背景と違和感（不自然さ）の根絶
1. **ヘッダーボタンのテキスト肥大化の解消**:
   - ヘッダー右上に「AI推薦の現状を無料診断」（11文字）というLPのキャッチコピー並みの長文が配置されており、アスペクト比が極端に崩れ、「黒い巨大な板」が浮いているような重苦しい違和感が生じていた。
   - 世界的SaaS（Linearの「Get started」、Vercelの「Sign Up」等）の鉄則に従い、ヘッダーCTAは**「無料診断」**（4文字）へ純化。視覚的な圧迫感を完全に排除。
2. **視覚的階層（Visual Hierarchy）とレイアウト分離の確立**:
   - 左側「診断レポートの見本」「AI推薦データの見本」「週次見守りの見本」「料金プラン」というサイト内ナビゲーション（`header-nav-links`）と、右側「管理画面を開く」「ログイン」「無料診断」というユーザーアクション（`header-nav-actions`）を構造的に分離。
   - ナビゲーションとアクションエリアの間に自動余白（`justify-content: space-between`）を配備し、誰が見ても1秒で構造が直感理解できる美しいヘッダーを完成。
3. **マイクロインタラクションとプロポーションの洗練**:
   - `.header-cta`: 高さ34px、左右パディング16px、角丸6px、フォントサイズ0.8rem（太字）。控えめなインナーハイライトと多層シャドウを施し、洗練されたクリック感を提供。
   - `.header-login-link`: ログインリンクに適切なパディング（`padding: 6px 14px`）とホバー時の微細な背景変化を付与し、押しやすく整った佇まいへ昇華。
   - `components/management-entry.tsx`: URL貼り付けだけでなく、「Google・メールアドレスでログインする →」セカンダリボタンを追加。認証と管理画面の動線を完全に一体化。

### 51.2 品質検証
- `npm test`: 全186件の自動テストすべてPass。
- `npx tsc --noEmit`: TypeScript型検査エラー0件。
- `npm run build`: Next.js全29ルートの静的生成・本番ビルド完全成功。

---

## 52. Google検索衰退セクション（購買行動変化）の弱腰コピー全面浄化と品格ある「3つの現実」への外科的昇華

2026-09-09、トップページ中段の「Google検索衰退・AI相談移行セクション（`components/google-decline-problem-section.tsx`）」において、おどおどした他人事の弱腰表現を完全に切除。客観的事実と損失回避性に直結する、重厚で品格ある「3つの現実」へと全面改修を実施。

### 52.1 課題と病巣の根本切除（「弱すぎる」の解剖）
1. **他人事の言い訳調・弱腰コピーの蔓延**:
   - 旧表記：「AI回答時代に確認しておきたいこと」「検索だけでなく、AIに直接相談して比較する場面もあります」「質問・参照元・測定時点によって表示される候補が変わります。ここでは、回答を測定する前に確認しておきたい3つのポイントを整理します。」
   - **分析**: まるで教科書のコラムのように他人事で、中小企業の社長が直面している「静かな顧客流出」の危機感や行動動機（サバンナOS・損失回避性）が1ミリも伝わっていなかった。
2. **胡散臭さゼロと品格の両立（誇大広告の完全排除）**:
   - 「売上10倍」「AI検索を完全支配」のような情報商材的・詐欺的な胡散臭い文言は一切使わず、**「検索窓からAIへの購買行動の地殻変動」**と**「AIが機械として客観検証できるデータを求めるメカニズム」**という冷徹な事実を論理的に提示。

### 52.2 外科的適正化の全容
1. **セクション見出し・リード文の昇華**:
   - 上部バッジ: `検索からAI相談へ：購買行動の地殻変動`
   - メインH2: `お客さんはGoogleで探すのをやめ、AIに「どこが良い？」と直接聞く時代へ。`
   - リード文: `広告や比較サイトが並ぶ検索結果よりも、中立で的確な答えをくれる生成AIへ相談する顧客が急速に増えています。しかし、AIが検証できる確定データがWeb上に整っていなければ、御社は候補にすら挙がらず、静かに競合や大手へ顧客が流出します。今、中小企業が直面している「3つの現実」を整理しました。`
2. **3大カードの再構造化（「理由」から「直面する現実」へ）**:
   - **現実 01**: `検索で1位でも、AIに無視されれば客は来ない`（見えない顧客流出: 自社サイトのアクセス数だけを見ていても、AIの段階でライバルに奪われた顧客には気付けない）
   - **現実 02**: `人間向けの「綺麗なホームページ」は、AIには届かない`（構造的な死角: ホームページを多額の費用でリニューアルしても、AIが解釈できる確定データがなければ推薦対象にならない）
   - **現実 03**: `対策を放置すると、大手やライバルへの送客が固定化する`（手遅れになる前に: AIの回答傾向が固定化する前に、AIが参照できる専用データを配備・維持することが不可欠）

### 52.3 品質検証
- `npm test`: 全186件の自動テストすべてPass。
- `npx tsc --noEmit`: TypeScript型検査エラー0件。
- `npm run build`: Next.js本番ビルド完全成功。

---

## 53. 全画面の弱腰・言い訳調・開発者目線コピーの全面浄化（完全放置・業種別シミュレーションのプロ水準昇華）

2026-09-09、サイト全体に残存していた「おどおどした弱腰表現」「開発者の内部変数・実装メモが漏れ出た不自然な日本語」「他人事のお茶濁しコピー」を徹底監査し、金融機関水準の品格と経営者に刺さる説得力を持つ言葉へ外科的に昇華。

### 53.1 監査で摘出した病巣と是正処置
1. **開発者内部仕様メモの漏出の根絶**:
   - 旧表記：「同意後は自動更新：有効な有料Watchで、同じサイトの短い記載を参照元付きで更新」
   - **分析**: 「有効な有料Watch」「同じサイトの短い記載」など、システム内部のフラグや変数名をそのまま画面に出したような不自然極まりない文章で、文系経営者を混乱させていた。
   - **是正**: 「**AI推薦データの自動更新**：自社情報の変化やAIの回答傾向に合わせ、公開データを自動で最新化」へ昇華。
2. **完全放置（Zero Effort）セクションの直言化**:
   - 旧表記：「AI推薦への取り組みを、少ない手間で」「情報の整備と毎週の追跡を、Rovanに任せて本業へ」「継続更新を許可します」「〜材料を集めるから」「〜ページを作れるから」
   - **是正**:
     - バッジ: 「**社長の作業ゼロ：完全放置（Zero Effort）の仕組み**」
     - メイン見出し: 「**URLまたは社名を入力するだけで、準備は完了。<br>情報の整備と毎週の追跡を、すべてRovanに任せて本業へ。**」
     - リード文: 「専門知識や面倒なアンケート回答は不要。公開情報からAI推薦データを自動構築し、初回確認だけで配備完了。」
     - 理由カード01: 「**既存の公開情報から、選ばれる理由を自動抽出**」
     - 理由カード02: 「**自社サイトの改修は1文字も不要。AI専用ページを開設**」
     - 理由カード03: 「**毎週のAI回答追跡と、掲載データの自動調整**」
3. **業種別シミュレーション（事例ギャラリー）の課題解決対比化**:
   - 旧表記：「公開情報を比較しやすく整える例」「〜比較しにくいことがあります」
   - **是正**:
     - 見出し: 「**町工場も、士業も、カフェも、農園も。<br>あらゆる業種で『選ばれるAI推薦データ』を即座に配備。**」
     - 各社ビフォー: 「小ロット対応の実績が埋もれ、AIが大手ばかり推薦してしまう」「設備情報が埋もれ、近くの作業カフェとして認識されない」など、経営者が直面する痛点を明確化。
     - 各社アフター: 「確定データとして配備し、急ぎの試作相談でAIの推薦候補へ浮上」「全席電源の確定データを配備し確実に選ばれる状態へ」と、具体的成果を端正に提示。
