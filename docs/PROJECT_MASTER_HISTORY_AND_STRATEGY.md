...
| **Phase 3** | **競合Web監視**<br>([`lib/autonomous-watch.ts`](file:///Users/satoushinya/project/AIX-next/lib/autonomous-watch.ts)) | `detectCompetitorWebChanges` を新設。競合のAI推薦シェア急増や新規台頭を `CompetitorEvent` として自動抽出。 | 経営者が気づかない水面下のライバルの動きをRovanが常時先回り監視。 |
...
| **Phase 5** | **再測定因果追跡**<br>([`lib/autonomous-watch.ts`](file:///Users/satoushinya/project/AIX-next/lib/autonomous-watch.ts)) | `evaluateAutoActionImpact` を新設。施策実行前後の観測ログから「観測された変化（Observed Uplift）」とAIプロバイダー合致度を客観算出。「Rovanのおかげで100%勝った」という誇大因果断定を排除。 | 信頼性の極限化。客観的な測定数値で社長に安心感と費用対効果を証明。 |
| **Phase 6** | **継続価値UI＆通知**<br>([`components/watch-client.tsx`](file:///Users/satoushinya/project/AIX-next/components/watch-client.tsx)<br>[`lib/watch-email.ts`](file:///Users/satoushinya/project/AIX-next/lib/watch-email.ts)) | カルテ画面に「THIS WEEK / 自律防衛」および「MONTHLY VALUE REPORT / 月次防衛総括（観測数・競合変動・Citation変動・台帳更新・Uplift・リスク追跡）」カードを設置。週次メールも「競合の動き → Rovanの自動対処 → 測定成果」の経営レポートフォーマットへ刷新。 | 「Rovanが勝手に守ってくれている」実感が毎週・毎月届き、解約する理由が完全に消滅。 |
...
### 46.1 絶対北極星：『AI顧客奪還シェア（Competitor Take-Back Share）』
1. **人間心理の急所（なぜこの指標なのか）**:
   - 人間は「100の利益（売上が増える）」より「100の損失（客を大手に奪われる）」を2.5倍恐れる（損失回避性）。
   - 「スコアが上がった」「売上が上がった」という曖昧・他責な数字を排除し、**「大手に流出していた買い手相談において、御社が身代わりとして最有力候補へ浮上した割合」**を唯一の北極星とする。
2. **測定ロジック：テレビ視聴率調査（ミステリーショッパー方式）**:
   - 一般人のプライベートチャットを盗聴することは技術的・法的に不可能。
   - したがって、「買い手の代表質問（固定50問パネル）」に対し、Rovanの監視ロボットが一般購入者のフリをして定期的に覆面調査を自動実行。
   - 「固定パネル50問中、大手を抑えて自社が推薦候補枠に入った割合（例: 0% ➔ 54%）」を客観的事実（Fact）として算出し、1本のグラフで追う。

### 46.2 大手との無謀な殴り合いの完全回避（ランチェスター戦略・弱者の戦法）
1. **大手の真似は即死を招く**:
   - 大手チェーン（カチタス等）が「全国展開・資本力・即日現金化」を打ち出してきた際、地域の中小企業が同じ「即日対応」で対抗することは物理的に不可能。
   - 社長に「御社もできますか？」と聞くこと自体が不快感と離脱を生む。
2. **AIの推薦メカニズムの本質（最適解の振り分け）**:
   - AIは「全員に大手チェーンを勧めたい」わけではなく、「相談者の文脈（条件）に応じた最適解」を探している。
   - 「早さと安さなら大手。だが、狭小地や親身な相談なら地元の専門企業」とAIに認識させることが必勝パターンとなる。

### 46.3 「社長は作業ゼロ」×「裏でのAI推薦データ蓄積」による自動局地戦防衛
1. **裏でのAI推薦ビッグデータ独占蓄積**:
   - Rovanは水面下で膨大なAI回答ログをクローリング・蓄積し、「AIが今どんな事実・エビデンスを好んで引用しているか」の勝ち筋パターンを独占保有する。
2. **勝手に局地戦仕様へ台帳を自動調整**:
   - 大手の動きを検知した際、大手と同じ土俵での殴り合いを完全回避。
   - 自社の既存公開情報から「地域密着・創業年数・自社施工・専門実績」など、大手には真似できないニッチ事実を自動抽出し、AI公式台帳のフォーカスを勝手に再最適化する。
3. **社長には「完了報告（感謝しろモデル）」だけを届ける**:
   - 社長への質問・確認・1タップ操作すら全廃。
   - **「大手他社はこういう戦略をとってきました。大手と殴り合わずに勝てるよう、御社既存の強みを元にAI公式データを調整しておきました（※社長の作業ゼロ）」**という参謀からの完了報告のみを届ける。
   - 社長は1文字も触っていないのに、「Rovanが裏で大手対策をやってくれた」という圧倒的安心感と誇りを獲得し、解約が心理的に完全に不可能となる。
