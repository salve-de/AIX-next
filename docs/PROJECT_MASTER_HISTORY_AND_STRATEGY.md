...
46. [旧案の記録：北極星と自動局地戦防衛（現行仕様では不採用）](#46-旧案の記録北極星と自動局地戦防衛現行仕様では不採用)
47. [現行仕様への訂正と北極星の確定](#47-現行仕様への訂正と北極星の確定)
...
| :--- | :--- | :--- | :--- |
| **Phase 2** | **公開台帳自動保守**<br>([`lib/storage.ts`](../lib/storage.ts)) | 旧案。定期スキャン時の自動更新を想定していたが、現行方針では公開プロフィールへの反映を承認前提にする。 | 過去の構想であり、現在の自動反映を意味しない。 |
| **Phase 3** | **競合Web監視**<br>([`lib/autonomous-watch.ts`](../lib/autonomous-watch.ts)) | 旧案。AI回答の候補増減だけでは競合Webの変更を証明できないため、現行実装は本文差分がない限りイベントを生成しない。 | 過去の構想であり、競合の常時監視や市場シェアを意味しない。 |
| **Phase 4** | **自律対応エンジン**<br>([`lib/autonomous-watch.ts`](../lib/autonomous-watch.ts)) | 旧案。現行実装は自社公開ページから確認できる事実を、公開前の確認案として扱う。 | 自動公開や優位性の保証を意味しない。 |
| **Phase 5** | **再測定因果追跡**<br>([`lib/autonomous-watch.ts`](../lib/autonomous-watch.ts)) | `evaluateAutoActionImpact` は比較可能な観測差分を記録するが、施策の因果効果は断定しない。 | 観測された差分と事業成果を混同しない。 |
| **Phase 6** | **継続価値UI＆通知**<br>([`components/watch-client.tsx`](../components/watch-client.tsx)<br>[`lib/watch-email.ts`](../lib/watch-email.ts)) | 同じ質問パネルのAI回答、参照元URL、確認案、再測定を表示する。競合Web変更や事業成果は、根拠がない限り表示しない。 | 過去の「自律防衛」「解約理由が消滅」などの表現は現行仕様ではない。 |
...
## 46. 旧案の記録：北極星と自動局地戦防衛（現行仕様では不採用）

> **履歴上の旧案**: この章は過去の検討内容を保存するための記録であり、現行仕様・提供保証・実装済み機能の説明ではない。以下に含まれる「AI顧客奪還シェア」「完全放置での自動補強」「顧客流出」などの表現は、現在は採用していない。現行の判断は第47章と `AGENTS.md`、`docs/CORE_PRODUCT_STRATEGY.md`、`docs/CONTINUOUS_VALUE_RETENTION.md` を参照すること。
...
> **旧版の結語（現行仕様では不採用）**: ここまでの旧記録にある「全戦略が完全実装・検証済み」という断定は、現在の証拠基準では採用しない。検証状態は第47.4節のとおり、処理・環境・外部サービスごとに個別確認する。

## 47. 現行仕様への訂正と北極星の確定

第46章までの記録には、検討段階の仮説や、現在は採用していない強いマーケティング表現が含まれる。本章を現行仕様の要約とし、過去の記録を実装済み・本番稼働済みの証拠として扱わない。

### 47.1 現行の北極星

現行の北極星は **候補回復率（測定質問ベース）** である。

```text
候補回復率 = 再測定で自社が候補に含まれた、初回候補外質問の数
           ÷ 初回に自社が候補外だった質問の数
```

同じ質問パネル、AIプロバイダー・モデル、地域、反復条件、測定期間をそろえた場合に限って比較する。この指標は顧客数、売上、検索順位、プロバイダー内部順位、市場シェア、因果効果を意味しない。無料確認では12問、週次測定では設定されたコアパネル（現行コードでは50問）を使うが、実行されたパネル数を必ず併記する。

### 47.2 現行の自動化・公開境界

- 公開ページのクロール、設定済みAIプロバイダーへの測定、出典URL付きの観測、差分整理、週次履歴の保存は、設定と実装範囲の中で自動化する。
- 対象サイトへの書き込み、第三者サービスへの申請、プロバイダー内部情報の変更、公開プロフィールや変更案の反映は自動実行しない。公開または反映には明示的な承認が必要である。
- 競合サイトの変更は、保存された前後クロールと再現可能な差分がある場合だけ観測事実として扱う。証拠がない場合に、競合の動き、対応完了、顧客の流出・回復を生成しない。
- 企業情報は公開情報の参照インデックスとして扱い、公式台帳、推薦保証、送客保証、売上保証とは称しない。架空サンプルはサンプルであることを画面上で明示し、実在企業の事実と混ぜない。
