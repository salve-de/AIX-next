...
Rovanが提供すべき最大の価値は、単なる「AI順位の診断」ではなく、次の運用を一度の委任後に継続して実行すること。

```text
一度だけ: URL・対象顧客・目標・事実・公開権限を接続
      ↓
Rovan: 買い手の質問を調査し、競合との差と根拠を測る
      ↓
Rovan: 最も売上に近い変更を作り、GitHub/CMSへレビュー可能な形で渡す
      ↓
承認された範囲: 公開、sitemap/IndexNow通知、計測設定を実行
      ↓
Rovan: AIの候補入り・引用・AI流入・問い合わせ・商談を同じ履歴で追う
      ↓
Rovan: 次に実施する一手を、見込める商談価値の順に選ぶ
```

これなら「使うだけで得られる価値」は、次のように説明できる。

> 競合に取られている買い手の質問を見つけ、根拠のある変更を公開し、AIから来た見込み客が商談・受注につながったかまで追う。毎週の調査と優先順位付けはRovanが行う。

ただし、定常作業ゼロと完全な無許可自動化は別物である。最初に所有権・接続・自動公開ポリシーを一度設定し、危険な変更は承認待ちにする。ここを省いて「お任せで必ず売上が上がる」と約束するのは、技術的にも契約・規約上も不正確である。

## 2. 目的をKPIの階層に戻す

AIの推薦順位は中間指標であり、Rovanの北極星は売上に近い順で置く。
...
| E | 受注、売上、粗利、継続 | 事業成果 | Rovanの一変更だけの因果（対照や検証なしでは不可） |
...
「ゼロ稼働」は、危険な操作を無断で行う意味ではなく、顧客が許可した範囲でRovanが定常運用を代行する意味に定義する。
...
**Rovanの実装**: Buyer Prompt → Evidence Gap → Change Pack → PR/CMS Draft → 再測定。
...
**なぜ効く可能性があるか**: Rovanの合成プロンプト観測だけでなく、実際の検索面で表示されたURL・国・端末・日付・引用状況を確認できる。
...
**何をするか**: Rovanが作ったChange Packを、GitHubのブランチ/PRまたはWordPress等のDraftとして作成し、差分・根拠・関連質問・公開前チェックを残す。
...
**自動化**: 低〜中。候補リスト、依頼文、事実台帳、掲載状況の監視は自動化できるが、第三者の掲載・レビューをRovanが作ってはいけない。
...
**限界**: Googleはローカル順位を主に関連性、距離、知名度/人気などで説明しており、支払いで順位を買う方法はない。Rovanはプロフィールを勝手に取得・変更しない。
...
**Rovanが表示するもの**: AI経由セッション、AI流入ページ、コンバージョン、案件、受注の観測値と帰属モデル。プロンプト順位と売上は別系列で並べ、因果を断定しない。
...
**Rovanの範囲**: 高意図質問・競合差・LP案・広告文案・計測設計。広告入稿や予算変更は別権限・明示承認にする。
...
Orbitrの公開ドキュメントは、`inspect → strategy → review/approve → execute → verify`を分離し、利用可能な操作はプラットフォーム、付与権限、証拠に依存すると明記している。これはRovanが採用すべき責任分界に近い。

さらに、実装先のAPIも「書ける」ことと「無条件に書いてよい」ことを分けている。GitHubはブランチ/PRを作成でき、WordPress REST APIはDraftやPublishの状態を持つ。RovanはデフォルトをPR/Draftにし、顧客が選んだ低リスク操作だけを別途自動化するのが安全である。
...
技術的には、OAuth/API/スニペット/エッジレイヤーを使えば自動反映できる。しかし、次を満たさない自動化はRovanでは採用しない。
...
Google Business Profileでは第三者が管理する場合も所有者の明示同意が必要で、レビュー返信には明示承認が必要とされる。第三者が「上位表示を保証する」と主張することも禁止されている。したがって、外部プロフィール・レビュー・PRまでRovanが無断で書く設計は、技術的に可能でも商品として不適切である。
...
したがって、現在のRovanを「使うだけで売上が上がる自動運転」と呼ぶのはまだ早い。現在は、損失の発見、根拠のある下書き、同じ質問での再測定までが実装された状態である。
...
- Rovanが毎週、変更を一つに絞って実装・確認し、重要な変化だけ通知する。
- 高リスク業界は専門担当者のレビューをサービスに含める。
- EC顧客だけUCP/商品フィードを別パッケージで扱う。

## 8. 料金・商品設計の仮説

価格は市場調査の確定値ではなく、現在のRovan機能から検証する仮説とする。

| 商品 | 顧客が買う結果 | Rovanの作業 | 位置づけ |
|---|---|---|---|
...
- 「RovanにURLを入れれば、全AIがあなたを推薦する」という保証。
...
- 単純な前後差をRovanの因果的な売上増加として報告すること。
...
この運用を実装して初めて、「Rovanに任せれば、顧客の定常稼働なしに、AI検索を含む需要獲得と売上改善を進める」という価値を提供できる。現時点のリポジトリはこのうち最初の診断〜下書き〜再測定までで、接続・公開反映・Outcome Ledger・運用代行は次段である。
